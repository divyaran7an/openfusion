...
**CLI version drift is handled, not hidden.** OpenFusion probes each installed CLI's help output once (`claude --help`, `codex exec --help`) and adapts its flags to what that build supports. On an older build that lacks a robustness flag (for example Claude's `--tools` or Codex's `--ignore-rules`), the run still works and `/api/health` reports the degradation as `cli_warnings` on the harness entry. Security-critical flags are never dropped: Codex always runs with `-s read-only`. Update the CLI to clear the warning, or point `FUSION_CLAUDE_CODE_COMMAND` / `FUSION_CODEX_COMMAND` at a newer build (they also accept a wrapper script, useful for pinning a specific install).
...
| `/api/health` shows `cli_warnings` on a harness | The installed CLI lacks a flag OpenFusion normally passes; runs work with the flag skipped | Update that CLI, or set `FUSION_CLAUDE_CODE_COMMAND` / `FUSION_CODEX_COMMAND` to a newer build |
