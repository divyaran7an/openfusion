...
test("nodeModelId namespaces harness sources but leaves gateway ids bare", () => {
...
test("graphToOverride emits per-node effort and web config aligned with the panel", () => {
  ...
  assert.deepEqual(override.panel_config, [
    { effort: "low", web: true },
    { effort: "high", web: undefined }
  ]);
  assert.deepEqual(override.judge_config, { effort: "minimal", web: undefined });
  assert.deepEqual(override.synth_config, { effort: "high", web: false });

  // Web tool configs attach because at least one node opted in.
...
test("graphToOverride omits web tool configs when no node enables the web", () => {
  const override = graphToOverride(
    graph([
      node({ id: "p", role: "panel", source: "gateway", model: "a" }),
      node({ id: "s", role: "synthesizer", source: "gateway", model: "b" })
